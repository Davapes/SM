...
<head>
    ...
    <script language="JavaScript">
        ...
        function editGoodsSell() {
            var allCheck=document.getElementsByName("gsID");
            var num=0;
            var gsID="";
            for(var i=0;i<allCheck.length;i++){
                if(allCheck[i].checked){
                    num++;
                    gsID=allCheck[i].value;
                }
            }
            if(num==1){
//                window.parent.frmMain.location="editBook.jsp?gsID="+gsID;
            }else if(num==0){
                alert("没有选中信息！");
                return;
            }else {
        ...
        function deleteGoodsSell() {
            ...
            if(num>0){
                if(window.confirm("您确定要删除选中的销售表单信息？")){
                    //创建一个XMLHttpRequest对象
                    CreateXMLHttpRequest();
                    xmlHttp.onreadystatechange=processor;

                    xmlHttp.open("Get","DeleteGoodsSellServlet?del="+delstr);
                    //发送请求
                    xmlHttp.send();
                }
            }else{
...
<body>
<%
    List<GoodsSell> goodsSellList=(List<GoodsSell>)request.getAttribute("goodSellList");
                ...
                <table border="0" align="left" cellspacing="0" class="list_table"
                    ...
                    <tr>
                        <td align="center" width="1%">
                            <input type="checkbox" name="userId" value="<%=staff.getgsID()%>">
                        </td>
